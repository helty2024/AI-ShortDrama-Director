...
for (const schema of [6, 7, 8]) test(`old v${schema} backup restores into v9 without inventing provenance`, async () => {
  ...
  try {
    ...
    if (schema >= 7) { const raw = new DatabaseSync(join(folder, 'project.sqlite')); raw.exec('BEGIN IMMEDIATE'); migrateGeneration(raw); raw.exec('COMMIT'); if (schema === 8) { raw.exec('PRAGMA foreign_keys=OFF; BEGIN'); migrateApproval(raw); raw.exec('COMMIT') } raw.close() }
