...
export class GenerationApprovalService {
  ...
  private checkPreflight(record: ProvenanceRecord, currency: string, ceiling: number, allowUnknown: boolean): void {
    ...
    if ((estimate.currency !== undefined && estimate.currency !== currency) || record.currency !== currency || (estimate.cost.status === 'known' && estimate.cost.estimatedCost.currency !== currency)) fail('currency-mismatch')
  ...
  reconcileSubmissionReceipt(p: string, id: string, raw: unknown): Reservation {
    return this.repository.atomic(() => {
      ...
      const r = this.repository.get('approval_reservations', p, id)
      if (!['submitted','pending-unknown'].includes(r.status) && !(receipt.data.id !== null && ['consumed','requires-review'].includes(r.status))) fail('reservation-conflict')
